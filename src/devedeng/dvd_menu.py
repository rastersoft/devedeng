...
class dvd_menu(devedeng.interface_manager.interface_manager):
    ...
    def write_text(self, text, text_type, xl, xr, y, alignment):
        ...
        if (bgcolor is not None):
            self.cr.move_to(x + extents[0] + 2, y -
                            (extents[3] / 2.0) - extents[1] + 2)
    ...
    def create_menu_stream(self, path, n_page, coordinates):
        ...
        for element in coordinates:
            ...
            if (element[4] == "left"):
                ...
                entry_data["right"] = "boton" + \
                    str(n_page) + "x" + str(counter)
            else:
                entry_data["chapters"].append(
                    "boton" + str(n_page) + "x" + str(counter))

            xml_file.write('\t\t\t<button name="boton' +
                           str(n_page) + "x" + str(counter))
            xml_file.write('" x0="' + str(xl) + '" y0="' + str(yt) +
                           '" x1="' + str(xr) + '" y1="' + str(yb) + '"')
            if counter > 0:
            ...
            if (element[4] == "left") and (has_next):
                xml_file.write(' right="boton' + str(n_page) +
                               'x' + str(counter + 1) + '"')
            if (element[4] == "right") and (has_previous):
                xml_file.write(' left="boton' + str(n_page) +
                               'x' + str(counter - 1) + '"')
